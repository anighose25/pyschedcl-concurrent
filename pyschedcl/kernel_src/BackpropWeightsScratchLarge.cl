...
void kernel backprop_floats_withscratch_dobias_striped(
        ...
        global float *Weightsupdate,
        #ifdef BIASED
             global float *gradBiasWeights,
             global float *Biasupdate,
        #endif
    ...
    for (int n = 0; n < batchSize; n++) {
        ...
        for (int stripe = 0; stripe < gNumStripes; stripe++) {
            ...
            for (int i = 0; i < numLoopsForErrorStripe; i++) {
                ...
                if (process) {
                    	//printf("gradoutput[%d] = %f\n",globalErrorsOffset, gradOutput[globalErrorsOffset]);
                ...
                for (int outRow = stripeOutRowStart; outRow < stripeOutRowEndExcl; outRow++) {
                    ...
                    for (int outCol = 0; outCol < gOutputSize; outCol++) {
                        ...
                        if (proceed) {
                            ...
                            //printf("ERROR Index: %d - %d * %d = %d,Value: %f\n",resultIndex,stripe,gOutputStripeSize, resultIndex - stripe * gOutputStripeSize,error);
  ...
  Weightsupdate[workgroupId * gFilterSizeSquared + localId] -= gradWeights[ workgroupId * gFilterSizeSquared + localId ];
    }
#ifdef BIASED
    ...
    if (writeBias) {
        ...
        Biasupdate[outPlane] -= gradWeights[ outPlane];
    }
#endif
    // gradWeights:     [outPlane][upstreamPlane][filterRow][filterCol]
    //       aggregate over:  [outRow][outCol][n]
