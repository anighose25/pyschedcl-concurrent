...
kernel void forwardNaive(const int N, global float *out, global const float *in) {
    const int globalId0 = get_local_id(0)*2;
    const int globalId1 = get_local_id(0)*2+1;
    if (globalId0 >= N) {
        return;
    }
    if (globalId1 >= N) {
    ...
    out[globalId0] = ACTIVATION_FUNCTION(in[globalId0]);
    out[globalId1] = ACTIVATION_FUNCTION(in[globalId1]);
}
#endif
