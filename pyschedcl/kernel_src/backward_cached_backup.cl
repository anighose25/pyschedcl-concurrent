...
void kernel calcGradInputCached( 
	...
	barrier(CLK_LOCAL_MEM_FENCE);
		...
		float thisWeightTimesError =  _gradOutputPlane[outRow * gOutputSize + outCol] *  _filterPlane[filterRow * gFilterSize + filterCol];
                    sumWeightTimesOutError += thisWeightTimesError;
                    //printf("%f %d \n" , gradOutputGlobal[(n * gNumFilters + outPlane) * gOutputSizeSquared + outRow * gOutputSize + outCol] , (n * gNumFilters + outPlane) * gOutputSizeSquared + outRow * gOutputSize + outCol);
                }
            }
        }
    }
    ...
    if (localId < gInputSizeSquared) {
        printf("%f\n",sumWeightTimesOutError);
	gradInput[upstreamImageGlobalOffset + localId] = sumWeightTimesOutError;
        #if RELU == 1
            gradInput[upstreamImageGlobalOffset + localId] = gradInput[upstreamImageGlobalOffset + localId]*(input[upstreamImageGlobalOffset + localId] >= 0);
        #endif
    } 
}
