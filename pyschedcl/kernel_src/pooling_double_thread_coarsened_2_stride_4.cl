...
kernel void forwardNaive(const int batchSize, global const float *input, global int *selectors, global float *output) 
{
    ...
    const int globalId0 = get_group_id(0)*get_local_size(0)+(get_local_id(0)/4)*4*2 +get_local_id(0)%4+0*4;

    const int globalId1 = get_group_id(0)*get_local_size(0)+(get_local_id(0)/4)*4*2 +get_local_id(0)%4+1*4;
