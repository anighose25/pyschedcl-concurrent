...
kernel void forwardNaive(const int batchSize, global const float *input, global int *selectors, global float *output) 
{
    ...
    const int globalId0 = get_group_id(0)*get_local_size(0)+(get_local_id(0)/64)*64*2 +get_local_id(0)%64+0*64;

    const int globalId1 = get_group_id(0)*get_local_size(0)+(get_local_id(0)/64)*64*2 +get_local_id(0)%64+1*64;
