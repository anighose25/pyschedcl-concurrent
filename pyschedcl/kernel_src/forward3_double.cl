...
void kernel forward_3_by_n_outplane(const int batchSize,
      global const float *images, global const float *filters, global const float *bias, 
    global float *output,
    ...
    if (localId < gOutputSizeSquared) {
        ...
        output[resultIndex]+= bias[outPlane];
	#if RELU == 1
		output[resultIndex] = ACTIVATION(output[resultIndex]);
	#endif
    }
}
