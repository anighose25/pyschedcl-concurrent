...
__kernel void myGEMM3(const int M, const int N, const int K,
    ...
    const int globalRow = TS*get_group_id(0) + row; // Row ID of C (0..M)
    const int globalCol = TS*get_group_id(1) + col; // Col ID of C (0..N)
    //printf("%d %d %d %d %d %d\n" , get_global_size(0)  , get_global_size(1) ,  get_local_size(0), get_local_size(1) , get_num_groups(0) , get_num_groups(1));
    //if(get_global_id(0) == 0)
    printf("ids  %d %d %d %d %d\n" , get_global_size(0)  , get_global_size(1) , M , N , K);
    //printf("%d\n",get_global_size(0));
    // Local memory to fit a tile of TS*TS elements of A and B
    __local float Asub[TS][TS];
    __local float Bsub[TS][TS];
    ...
    for (int t=0; t<numTiles; t++) {
        ...
        for (int w=0; w<WPT; w++) {
            const int tiledRow = TS*t + row;
            const int tiledCol = TS*t + col;
            Asub[col + w*RTS][row] = A[(tiledCol + w*RTS)*K + globalRow];
            Bsub[col + w*RTS][row] = B[(globalCol + w*RTS)*M + tiledRow];
           //printf("B =%d  A = %d N = %d K = %d M = %d\n" , (globalCol + w*RTS)*K + tiledRow , (tiledCol + w*RTS)*M + globalRow , N , K , M);
        }
        ...
        for (int k=0; k<TS; k++) {
            for (int w=0; w<WPT; w++) {
                acc[w] += Asub[m][row] * Bsub[col + w*RTS][n];
            }
        }
    ...
    for (int w=0; w<WPT; w++) {
        C[(globalCol + w*RTS)*N + globalRow] = acc[w];
        //printf("store %d %f %f\n" , (globalCol + w*RTS)*M + globalRow ,C[(globalCol + w*RTS)*M + globalRow] , acc[w]);
    }
}
...
__kernel void myGEMM11(const int M, const int N, const int K,
    ...
    for (int y=0; y<RY; y++) {
        ...
        if(get_group_id(0) == 0)
        printf("%d %d %d %lf \n", M , N , (y + RY*get_global_id(1)) * (M/RX) + get_global_id(0) , C[(y + RY*get_global_id(1)) * (M/RX) + get_global_id(0)]);
