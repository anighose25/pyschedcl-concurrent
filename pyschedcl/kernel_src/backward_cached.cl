...
void copyLocal(local float *target, global float const *source, int N , int type) {
...
void kernel calcGradInputCached( 
    ...
    for (int outPlane = 0; outPlane < gNumFilters; outPlane++) {
        ...
        copyLocal(_filterPlane, filtersGlobal + (outPlane * gInputPlanes + upstreamPlane) * gFilterSizeSquared, gFilterSizeSquared , 1);
        copyLocal(_gradOutputPlane, gradOutputGlobal + (n * gNumFilters + outPlane) * gOutputSizeSquared, gOutputSizeSquared , 2);
        barrier(CLK_LOCAL_MEM_FENCE);
