#include "thrud/BasicBlockFeatures.h"
...
cl::opt<std::string> BBKernelName("bb-kernel-name", cl::init(""),
                                    cl::Hidden,
                                    cl::desc("Name of the kernel to analyze"));

extern cl::opt<unsigned int> CoarseningDirectionCL;

char BBFeatures::ID = 0;
static RegisterPass<BBFeatures>
    X("bb-features", "Collect opencl features in each basic block");


//------------------------------------------------------------------------------
bool BBFeatures::runOnFunction(Function &function) {
  if (function.getName() != BBKernelName)
    return false;
...
//------------------------------------------------------------------------------
void BBFeatures::getAnalysisUsage(AnalysisUsage &AU) const {
...
#define HANDLE_INST(N, OPCODE, CLASS)                                          \
  void BBFeatures::visit##OPCODE(CLASS &I) {                   \
...
//------------------------------------------------------------------------------
void BBFeatures::visitInstruction(Instruction &inst) {
  errs() << "Unknown instruction: " << inst;
  llvm_unreachable(0);
}

//------------------------------------------------------------------------------
void BBFeatures::visitBasicBlock(BasicBlock &basicBlock) {
...
//------------------------------------------------------------------------------
void BBFeatures::visitFunction(Function &function) {
