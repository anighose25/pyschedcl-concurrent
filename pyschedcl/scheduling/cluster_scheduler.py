...
if __name__ == '__main__':
    ...
    for i,dag_json_file in  enumerate(all_dags_jsons):
        if dag_json_file.endswith('json'):
            ...
            gpus=gpus,cpus=cpus,ctxs=ctxs,cmd_qs=cmd_qs,ex_stats_file=ex_stats,heft_scheduler=True)) #create dag for info file (ex :- dag_test1/t1.json)
    ...
    while True:
        ...
        logging.info("Dispatching task " + str(next_task.id))
        if args.use_thread:
    ...
    for dag in all_dags:
        ...
        for kernel_id,kernel in dag.kernels.items():
            ...
            kernel_timestamps["write"] = {"host_queued":kernel.host_events[0].write_start,\
            "device_queued":-1,"device_start":-1,"device_end":-1}
...
#            ev = kernel.nd_range_event
        ...
        dag.dump_graph("logs/start_end_times.png",rank=False,timestamps=timestamps)
        dag.dump_graph("logs/execution_times.png", rank=False,timestamps=None,print_all_times=True)
