...
def select_main(kernels,gpu = 1, ncpu = 1,select=baseline_select):
    cmd_qs, ctxs, gpus, cpus = fw.host_initialize(ngpu, ncpu)
    ...
    while (len(cpu_q) > 0 or len(gpu_q) > 0 or len(mixed_q) > 0 or num_dispatched != num_kernels) and (len(cpu_q) > 0 or len(gpu_q) > 0 or
        ...
        if fw.nCPU > 0 or fw.nGPU > 0:
            ...
            #     pass
                ...
                start_time, done_events = kernels[i].dispatch(gpu, cpu, ctxs, cmd_qs, C_cpu=1,C_gpu=1)
...
if __name__ == '__main__':
    ...
    if args.select == "baseline":
        dump_dev = select_main(kernels, ngpu, ncpu, select=baseline_select)
    if args.select == "lookahead":
        dump_dev = select_main(kernels, ngpu, ncpu, select=look_ahead_select)
    if args.select == "adbias":
        dump_dev = select_main(kernels, ngpu, ncpu, select=adaptive_bias_select)
